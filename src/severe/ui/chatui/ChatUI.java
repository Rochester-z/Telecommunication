...
import severe.thread.ReadThread;
import severe.ui.common.Emoticon;
import severe.ui.common.MyScrollBarUI;
...
//      与好友建立通信，开启通信线程，调用start方法是线程方法（可不带while），否则是普通方法
        ...
        //单例模式创建唯一通信线程
        MsgServerThread msgServerThread = MsgServerThread.getsingleton();
        //将发送框接收框传输给通信线程
        msgServerThread.setInputTextPane(inputTextPane);
        msgServerThread.setHistoryJTextPane(historyJTextPane);
        //将发送按钮传输给通信线程
        msgServerThread.setSendButton(sendButton);
        //将好友名传给通信线程
        String friendname = friend.getNickname();
        msgServerThread.setFriendname(friendname);
...
//                  image.requestFocus();
        ...
        video.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                System.out.println("服务端开启视频");
                super.mouseClicked(e);
                //启动视频线程
                VideoChatUIServer videoChatUIServer = new VideoChatUIServer();
